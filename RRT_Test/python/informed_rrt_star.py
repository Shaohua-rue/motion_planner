...
# x_start, y_start = width//2 -1500, height//2 
# x_goal, y_goal = width//2 +1500, height//2 

step_size = 100
distance_threshold = 1
# search_radius = step_size + distance_threshold
search_radius = 200
    ...
    if valid_node(x_node, y_node):
        ...
        if valid_node(x_new, y_new):
            ...
            if not reached:
                ...
                if np.sqrt((x_new-x_goal)**2 + (y_new-y_goal)**2) < 100:
...
# Mark the final node in red
canvas = cv2.circle(canvas, (x_goal, y_goal), 5, (0, 0, 255), 10)
# Mark the start node in green
canvas = cv2.circle(canvas, (x_start, y_start), 5, (254, 0, 0), 10)
