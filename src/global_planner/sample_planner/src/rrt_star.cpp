...
RRT_Star::RRT_Star(Vector2d _start, Vector2d _goal, vector<vector<double>>& obs, vector<vector<bool>>& obstacle_map_,Vector2d rand_area,
...
vector<vector<double>> RRT_Star::planning(void){
    ...
    for(int iterator  = 0; iterator < max_iter; iterator++){
        ...
        if(check_collision(new_node)){
            ...
            if(new_parent != nullptr && check_collision(new_parent)){
                ...
                std::cout << "RRT_Star: 节点更新" << std::endl;
...
***/
shared_ptr<Node> RRT_Star::get_random_node(void) {
...
*
 */
shared_ptr<Node> RRT_Star::get_nearest_node(Node rnd_node) {
...
*/
shared_ptr<Node>  RRT_Star::steer(shared_ptr<Node>  from_node, shared_ptr<Node>  to_node) {
...
*/
Vector2d RRT_Star::calc_distance_and_angle(shared_ptr<Node> from_node, shared_ptr<Node> to_node) {
    Vector2d d_angle;
...
*/
bool RRT_Star::check_collision(shared_ptr<Node> new_node) {
    if (new_node == nullptr) {
        return true;
    }
...
*@brief final path
*/
vector<vector<double>> RRT_Star::generate_final_course(void) {
...
*@brief draw_graph
*/
void RRT_Star::draw_graph(Node rnd) {
    plt::clf();
...
*
*/
vector<int> RRT_Star::get_neighbor_nodes(shared_ptr<Node> node)
{
...
**/
shared_ptr<Node> RRT_Star::get_new_parent(shared_ptr<Node> new_node,vector<int> neighbor_ids)
{
...
void RRT_Star::rewire(shared_ptr<Node> new_node,vector<int> neighbor_ids)
{
...
void RRT_Star::propagateCostToLeaves(shared_ptr<Node>parent_node) {
